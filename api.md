...
```
     Запрос на оставление заказа межгород для клиента

     path:     Base URL  + make-intercity-order/
     Method: POST
    ...
    fields: 
    {
        ...
        "start_id", -- место отправления ID города или поселка (НЕ РЕГИОНА)
        "end_id"  -- место прибытия ID города или поселка (НЕ РЕГИОНА)
...
```
Запрос на принятие заказа межгород для водителя

path:     Base URL  + accept-intercity-order/
Method: POST
...
```
Запрос на добавление нового заказа межгород для водителя

path:     Base URL  + add-intercity-order/
Method: POST
...
fields: 
{
    ...
    "end_id", -- место прибытия ID города или поселка (НЕ РЕГИОНА)
    "date",
    "price" -- если система сама не назначит цену
}
response:
{
    "state"
...
```
Запрос на получение цены на межгород для клиента или водителя

path:     Base URL  + get-intercity-price/
Method: POST
...
{
    "state"
    если прилетит пустая цена значит клиент или водитель могут назначить свою цену за ОДНО место в машине и отправить вместе с оформлением заказа параметр price 
}
